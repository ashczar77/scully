# Pragtig

Pragtig, pronounced **prakh-tik**, means **gorgeous** in Afrikaans.

Pragtig is an explainable application framework for Java and Kotlin that
brings productive automation to JVM backend services without opaque runtime
behavior.

The project is currently in its research phase. See the
[project documentation](docs/README.md) for its status, decisions, and roadmap.

## License

Pragtig is licensed under the [Apache License 2.0](LICENSE).
